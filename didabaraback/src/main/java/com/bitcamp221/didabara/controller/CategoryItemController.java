...
//  -----------------------------------------------------
    ...
    public ResponseEntity<?> create(@AuthenticationPrincipal final String userId,
        ...
        try {
            ...
            log.info(categoryItemDTO.toString());
            ...
            if (userId != null && Long.valueOf(userId) == categoryService.findHost(categoryId) && categoryItemDTO != null && file.getSize() != 0) {
                ...
                categoryItemDTO.setPreview(String.valueOf(filePath.get(0)));
                ...
                if (userId == null) {
                    throw new RuntimeException("1.유저 아이디 에러");
                } else {
                    throw new RuntimeException("2.파일 업로드 에러");
                }
