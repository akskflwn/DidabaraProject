...
public interface EmailConfigMapper {
    ...
    @Insert("INSERT INTO emailconfig VALUES(#{id},current_date(),current_date(),#{code})")
    int saveUserIntoEmailconfig(@Param("id") Long id, @Param("code") String code);

    @Update("UPDATE emailconfig SET auth_code = #{code} WHERE id=#{id}")
    int updateUserIntoEmailconfig(@Param("id") Long id, @Param("code") String code);
